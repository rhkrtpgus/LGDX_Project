...
export type FamilySelectionPreference = {
  familyId: number
...
export function fetchFamilySelectionPreference(familyId: number) {
  return request<FamilySelectionPreference>(
    buildUrl(javaApiBaseUrl, `/parent/selection?familyId=${familyId}`),
  )
}

export function updateFamilySelectionPreference(payload: {
  familyId: number
  selectedChildId: number | null
}) {
  return request<FamilySelectionPreference>(buildUrl(javaApiBaseUrl, '/parent/selection'), {
    method: 'PATCH',
    body: JSON.stringify(payload),
  })
}
